...
const heartDot = document.querySelector("nav .fa-sharp").after

let heartLsData = JSON.parse(localStorage.getItem("heart-data")) || []
display(heartLsData)
if(heartLsData.length<=0){
...
function display(data) {
    ...
    data.forEach((item) => {
        ...
        heart.setAttribute("class", "wishDeleteButton")
        heart.innerText="Remove"
        heart.addEventListener("click", () => {
            heartLsData = heartLsData.filter((i) => i.id !== item.id)
